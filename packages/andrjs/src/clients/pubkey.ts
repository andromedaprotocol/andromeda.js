...
function isEthSecp256k1Pubkey(pubkey: Pubkey) {
    return pubkey.type === "/ethermint.crypto.v1.ethsecp256k1.PubKey";
}

function isInjSecp256k1Pubkey(pubkey: Pubkey) {
    return pubkey.type === "/injective.crypto.v1beta1.ethsecp256k1.PubKey";
}
...
export function encodePubkey(pubkey: Pubkey): Any {
    ...
    } else if (isInjSecp256k1Pubkey(pubkey)) {
        const pubkeyProto = CosmosCryptoSecp256k1Pubkey.fromPartial({
            key: fromBase64(pubkey.value),
        });
        return Any.fromPartial({
